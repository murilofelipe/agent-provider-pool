...
export class GroqProvider {
    ...
    async complete({ prompt, model, temperature, image }) {
        ...
        if (image)
            throw new Error(`GroqProvider does not support image input (model ${model})`);
        ...
        if (res.status === 429)
            throw new QuotaExceededError(`Groq rate limit hit for model ${model}`);
        if (!res.ok)
            throw new Error(`Groq request failed (${res.status}): ${await res.text()}`);
        const body = (await res.json());
        const text = body.choices[0]?.message.content;
        if (text === undefined)
            throw new Error('Groq response had no completion content');
        return { text };
    }
}
