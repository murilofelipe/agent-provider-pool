...
interface QuotaRecord {
  day: string; // YYYY-MM-DD (UTC) -- dailyCount resets when this changes
  dailyCount: number;
  month: string; // YYYY-MM (UTC) -- monthlyCount resets when this changes
  monthlyCount: number;
  /** ISO timestamp. Set on a reactive 429 even if the configured limit
   * hadn't nominally been reached -- covers a wrong/stale configured
   * number. Cleared naturally once `day` rolls over. */
  exhaustedUntil?: string;
}

type QuotaFile = Record<string, Record<string, QuotaRecord>>; // providerName -> model -> record
...
export class QuotaStore {
  ...
  private load(): QuotaFile {
    ...
    try {
      return JSON.parse(readFileSync(this.path, 'utf-8')) as QuotaFile;
    } catch {
      return {};
    }
  }
  ...
  private record(providerName: string, model: string): QuotaRecord {
    const providerRecords = (this.data[providerName] ??= {});
    const existing = providerRecords[model];
    const day = today();
    const month = thisMonth();
    if (existing && existing.day === day && existing.month === month) return existing;
  ...
  isExhausted(providerName: string, config: ModelConfig): boolean {
    ...
    if (record.exhaustedUntil && record.exhaustedUntil > new Date().toISOString()) return true;
    if (config.dailyLimit !== undefined && record.dailyCount >= config.dailyLimit) return true;
    if (config.monthlyLimit !== undefined && record.monthlyCount >= config.monthlyLimit) return true;
    return false;
  }
